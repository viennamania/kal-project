...
export function HomeScreen() {
  ...
  const myTokens = communityTokens.filter(
    (token) => token.ownerWallet.toLowerCase() === account?.address?.toLowerCase()
  );
              ...
              {communityTokens.slice(0, 6).map((token) => (
                ...
                >
                  ...
                  <div className="mt-4 flex flex-wrap gap-2">
                    ...
                    {account?.address?.toLowerCase() === token.ownerWallet.toLowerCase() ? (
                      <Link
                        className="inline-flex items-center justify-center rounded-full border border-white/70 bg-white/80 px-3 py-2 text-xs font-semibold text-ink/80"
                        href={`/studio/${token.contractAddress}/manage`}
                      >
