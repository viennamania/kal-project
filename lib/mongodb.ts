  ...
  if (!indexesPromise) {
    indexesPromise = (async () => {
      ...
      await Promise.all([
        ...
        db.collection<InviteClaimDocument>("invite_claims").createIndex({ senderWallet: 1, createdAt: -1 }),
        db.collection<InviteClaimDocument>("invite_claims").createIndex({ targetPhoneHash: 1, createdAt: -1 }),
        db.collection<InviteClaimDocument>("invite_claims").createIndex({ tokenAddress: 1, createdAt: -1 }),
        db.collection<InviteClaimDocument>("invite_claims").createIndex({ status: 1, expiresAt: 1 }),
...
export async function getCollections() {
  ...
  return {
    ...
    inviteClaims: db.collection<InviteClaimDocument>("invite_claims"),
